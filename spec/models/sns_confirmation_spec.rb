require "spec_helper"

describe SNSConfirmation do
  describe "confirm" do
    before(:each) do
      @topicArn = "arn:aws:sns:us-east-1:377092858912:Teebox_processing_DEV"
      @token = ""
      @confirm = stub(topic: @topicArn, token: @token)
    end
    
    it "does something" do
      #SNSConfirmation.should_receive(confirm: @confirm).and_return(true)
      #.confirm(@topicArn, @token).should be_a_kind_of(Core::Response)
    end
  end
end