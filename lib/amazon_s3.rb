class AmazonS3
  ...
  def self.get_screenshots
     screenshots = Video.all.map(&:screenshot).map {|file| file.to_s.gsub!("https://#{CONFIG[:s3_bucket]}.s3.amazonaws.com/", "") }
     files = []
   ...
   def self.get_videos
     Video.all.map(&:file).each {|file| file.gsub!("http://#{CONFIG[:s3_bucket]}.s3.amazonaws.com/", "") }
   end
